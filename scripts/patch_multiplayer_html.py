...
def patch_balatro() -> None:
    ...
    if "OctopusLaunch" not in text or "mode-paperback" not in text or "mode-bunco" not in text or "mode-pokermon" not in text or "mode-multiplayer" not in text:
        raise RuntimeError("balatro.html is missing the Vanilla/Paperback/Bunco/Pokermon/Multiplayer launcher")
    text = re.sub(
...
def patch_index() -> None:
    ...
    if "OctopusLaunch" not in text or "mode-paperback" not in text or "mode-bunco" not in text or "mode-pokermon" not in text or "mode-multiplayer" not in text:
        raise RuntimeError("index.html is missing the Vanilla/Paperback/Bunco/Pokermon/Multiplayer launcher")
    if re.search(
