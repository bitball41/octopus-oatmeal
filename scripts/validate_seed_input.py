#!/usr/bin/env python3
"""Exercise the packed Lua 5.1 seed input callbacks."""
import sys
import zipfile
...
for name in z.namelist():
    if name.endswith('.lua'):
        ...
        assert ok, err
print('All packed Lua modules compile under Lua 5.1')
...
# Use the native eight-character seed editor configuration.
lua.execute("""
reset_input(true)
local config = G.CONTROLLER.text_input_hook.config.ref_table
config.extended_corpus = false; config.max_length = 8
love.textinput('aa12bb34zz'); assert(value() == 'AA12BB34', 'seed length/case failed')
press('left'); press('backspace'); assert(value() == 'AA12BB4', 'seed cursor deletion failed')
press('right'); press('delete'); assert(value() == 'AA12BB4', 'delete beyond end changed seed')
press('enter'); assert(G.CONTROLLER.submitted, 'seed submission failed')
love.textinput('ignored')
reset_input(true)
config = G.CONTROLLER.text_input_hook.config.ref_table
config.extended_corpus = false; config.max_length = 8
love.textinput('0![] abc'); assert(value() == 'OABC', 'native seed alphabet changed')
press('escape'); assert(not G.CONTROLLER.text_input_hook, 'escape failed')
""")
print('Seed editor: eight-character limit, alphabet, cursor editing, submit and escape passed')
