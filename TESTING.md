...
- **Modded** loads root `game.data` (Steamodded + Multiplayer). IndexedDB database `/home/web_user/love` so existing browser saves keep working.

Both modes mount the LÖVE save directory at `/home/web_user/love`. They only split the IndexedDB name, so a vanilla run cannot read a modded profile and the other way around.
